...
from LJPapp.model.TopMergeModel import TopMergeModel
from LJPapp.tool.test_tool import test


def t_predicate(fact):
    model_path = "LJPapp/model/11.pkl"
    bert_tokenizer_path = "LJPapp/bert-chinese/vocab.txt"
    bert_path = "LJPapp/bert-chinese"
    batch_size = 32
    learning_rate = 1e-4
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    state = torch.load(model_path)
    model = TopMergeModel(bert_path, batch_size, device).to(device)
    ...
    f = open('LJPapp/content/article_content_dict.pkl', 'rb')
    content = torch.load(f)
    results = "指控预测：犯" + "、".join(zm)
    results = results + "。  法条预测：(违反下列刑法条例) "
    for i in ft:
        results = results + i + "：" + content[i] + " "
    return results
