...
article = {'第347条': 0, '第264条': 1, '第383条': 2, '第234条': 3, '第133条': 4, '第196条': 5, '第280条': 6, '第345条': 7,
...
accusation = {'盗窃罪': 0, '走私、贩卖、运输、制造毒品罪': 1, '故意伤害罪': 2, '抢劫罪': 3, '诈骗罪': 4, '受贿罪': 5, '寻衅滋事罪': 6, '危险驾驶罪': 7,
...
def predicate_process(outputs, model):
    ...
    if model == "zm":
        for i in range(nr_classes):
            if outputs[0, i] >= 0.5:
                name = list(accusation.keys())[list(accusation.values()).index(i)]
                zm.append(name)
        return zm
    if model == "ft":
        for i in range(nr_classes):
            if outputs[0, i] >= 0.5:
                item = list(article.keys())[list(article.values()).index(i)]
                ft.append(item)
        return ft
